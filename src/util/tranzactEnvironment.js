...
const PY_FOLDER = '../tranzact/daemon';
...
const getTranzactVersion = () => {
  let version = null;
  const exePath = getExecutablePath('tranzact');
  // first see if we can get a tranzact exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or tranzact is on the path
    try {
      version = child_process
