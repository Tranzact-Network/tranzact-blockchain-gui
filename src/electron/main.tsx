...
import tranzactEnvironment from '../util/tranzactEnvironment';
import tranzactConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!tranzactEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    tranzactConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!tranzactConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !tranzactConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (tranzactConfig.manageDaemonLifetime()) {
        tranzactEnvironment.startTranzactDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Tranzact Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Tranzact-Network/tranzact-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Tranzact-Network/tranzact-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Tranzact-Network/tranzact-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Tranzact-Network/tranzact-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Tranzact-Network/tranzact-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/tranzact_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/tranzact_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Tranzact Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Tranzact' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Tranzact Blockchain' }),
            click: () => {
              openAbout();
            },
          },
