...
import { AlertDialog } from '@tranzact/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      tranzactper_input.value === '' ||
      Number(tranzactper_input.value) === 0 ||
      !Number(tranzactper_input.value) ||
      isNaN(Number(tranzactper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const tranzactper = tranzact_to_mojo(tranzactper_input.value);
    const tranzactper_value = Number.parseInt(Number(tranzactper));
    ...
    // var fee = tranzact_to_mojo(fee_input.value);
