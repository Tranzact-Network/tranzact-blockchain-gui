...
export default function Block() {
  ...
  const poolReward = toto_to_tranzact(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = toto_to_tranzact(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const tranzactFees = blockRecord.fees
    ? toto_to_tranzact(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.tranzactexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.tranzactexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: tranzactFees ? `${tranzactFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Tranzact blockchain
            </Trans>
          </BlockTitle>
        }
        action={
